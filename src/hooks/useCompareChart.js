...
function drawCategory(svg, actuals, budget, setTooltip, year) {
  ...
  const allVals = COMPARE_CATEGORIES.flatMap(cat => [budget[cat] ?? 0, actuals[cat] ?? 0]);
  ...
  const lgG = g.append('g').attr('transform', `translate(${C_IW / 2 - 108},${C_IH + 36})`);
...
function drawTotal(svg, actuals, budget, setTooltip, year) {
  ...
  const maxVal = Math.max(budgetTotal, actualTotal);
  ...
  // Y axis labels (manual — colored to match bars)
  g.append('text')
